<template>
  <div class="wx-main-view">
    ...
    <grid :cols="3">
      <grid-item v-for="(i,index) in initData"
                 :key="i"
    ...
    <grid :cols="3">
      <grid-item v-for="(i,index) in customData"
                 :key="i"
                 @on-item-click="onItemCustom(index)">
        <span class="grid-center">{{i}}</span>
      </grid-item>
    </grid>
    ...
    <v-activity-tip v-if="showModal"
                    v-on:close="close"
                    v-on:goto="goto"></v-activity-tip>
    ...
    <v-input-alert v-if="isShowPayTip"
                   v-on:cancelevent="cancel"
                   v-on:sureevent="sure"></v-input-alert>
